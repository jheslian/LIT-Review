...
def flux(request):
    try:
        ...
        tickets = Ticket.objects.filter(
            Q(has_review=False) & (Q(user=request.user) | Q(user__in=users)))
        reviews = Review.objects.filter(Q(user=request.user) | Q(user__in=users))
    except:
        tickets = Ticket.objects.filter(Q(has_review=False) & Q(user=request.user))
...
def follow_users_view(request):
    ...
    if request.POST:
        ...
        try:
            ...
            form.save()
            messages.success(request, f"Vous avez bien ajouté {request.POST['following']} sur votre reseau.")
        except:
